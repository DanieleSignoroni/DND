...
public class YAzioniPianoPrelievo extends AzioniPianoPrelievo{
	...
	/**
	 * Estraggo solo ordini con STATO_SCHEDUL = {@value OrdineEsecutivoPO#PROGRAMMATO} solo se 
	...
	public String costruisciWhere() {
		...
		if(isSoloOrdiniProgrammati()) {
			where += " AND " + WrapperPianoPrlDatiTM.STATO_SCHEDUL + " = '"+OrdineEsecutivo.PROGRAMMATO+"' ";
		}
		return where;
	}	
	...
	public CausaleDocumentoVendita getCausaleDocVen(OrdineVendita ordine, ClienteVendita cliente, String idDivisione) {
	...
	protected DocumentoVendita cercaDocumentoVenditaAccorpabile(OrdineVenditaRiga ordVenRig) {
		...
		if(ordVenRig.getDataConsegnaConfermata() != null)
			trovaDocVenAccodabile += " AND "+DocumentoVenditaTM.DATA_CONSEG_CFM+" = '"+ConnectionManager.getCurrentDatabase().getLiteral(ordVenRig.getDataConsegnaConfermata())+"' ";
