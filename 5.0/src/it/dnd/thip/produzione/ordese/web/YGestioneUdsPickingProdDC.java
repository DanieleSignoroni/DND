 ...
 * 71994	10/06/2025	DSSOF3	 Nuova gestione copia in accodamento
 */
...
public class YGestioneUdsPickingProdDC extends WebEnhDataCollector {
	...
	protected Vector runCheckAll() {
		if(getMode() == OpenType.NEW) {
			...
			getComponentManager("IdUds").setRunCheckRelatedBo(false); //.Per evitare che faccia il controllo sulla tabella relazionata
		}else if(getServletEnvironment() != null && (getServletEnvironment().getRequest().getParameter("AccodamentoUdsRaccoltaDati") != null
				&& getServletEnvironment().getRequest().getParameter("AccodamentoUdsRaccoltaDati").equals("Y"))
				&& getMode() == OpenType.COPY) {
			getComponentManager("IdUds").setRunCheckRelatedBo(false); //.Per evitare che faccia il controllo sulla tabella relazionata
		}
