...
public class YRilevDatiPrdTSDataCollector extends RilevDatiPrdTSDataCollector {
	...
	protected Vector runCheckAll() {
		...
		if(action != null && action.equals(YRilevDatiPrdTSFormActionAdapter.CONFERMA_CHIUSURA_LISTA_TESTA)) {
			getComponentManager("CodiceTestataLista").setMandatory(true);
			getComponentManager("IdOperatore").setCheckMode(BaseBOComponentManager.CHECK_NEVER);
			getComponentManager("IdOperatore").setMandatory(false);
		}
