...
import it.dnd.thip.logis.lgb.TipoMissionePianoCaricoRigaToyota;
...
public class ConfermaChiamataUDC extends Check {
	...
	public void actionOnObject(BODataCollector boDC, ServletEnvironment se) {
		...
		if(action.equals(YRilevDatiPrdTSFormActionAdapter.CONFERMA_CHIAMATA_UDC)) {
			...
			String idArticolo = (String) boDC.getComponent("IdArticolo").getValue();
			...
			boDC.getComponent("IdArticolo").setValue("");
			...
			if (boDC.check() != BODataCollector.OK) {
				...
				bo.setIdArticolo(idArticolo);
				bo.setIdOperatore(idReparto);
				if(bo.getIdArticolo() != null && (udc == null || udc.isEmpty())) {
					...
					if(mappa == null) {
						...
						try {
							...
							if(((YReparto)pianoCarico.getReparto()).isAgvNonDefault())
								pianoCarico.setStatoGestione(TipoGestioneUbicazione.MULETTISTA);
							else
								pianoCarico.setStatoGestione(((YUbicazione)mappa.getUbicazione()).getTipoGestionePers());
			...
			boDC.setOnBORecursive(); //.Scarico i nuovi attributi sul bo
			...
			try {
				...
				if(saldo != null) {
					...
					pianoCarico.setStatoGestione(((YUbicazione)saldo.getUbicazione()).getTipoGestionePers());
				}
				if(bo.getNumeroRitornoVersamento() != null) {
					pianoCarico.setNumeroRitornoVersamento((String) bo.getNumeroRitornoVersamento());
					String where = " "+AttivitaEsecutivaTM.ID_AZIENDA+" = '"+Azienda.getAziendaCorrente()+"'";
					where += " AND "+AttivitaEsecutivaTM.NUM_RITORNO+" = '"+bo.getNumeroRitornoVersamento()+"'";
					where += " AND "+AttivitaEsecutivaTM.ATV_FINALE+" = '"+Column.TRUE_CHAR+"'";
					Vector atvs;
					try {
						atvs = AttivitaEsecutiva.retrieveList(AttivitaEsecutiva.class, where, "", false);
						if(atvs.size() > 0) {
							AttivitaEsecutiva attivita = (AttivitaEsecutiva) atvs.get(0);
							YPianoCaricoToyotaRiga riga = (YPianoCaricoToyotaRiga) Factory.createObject(YPianoCaricoToyotaRiga.class);
							riga.setTestata(pianoCarico);
							riga.setTipoMissione(TipoMissionePianoCaricoRigaToyota.VERSAMENTO);
							riga.setArticolo(attivita.getOrdineEsecutivo().getArticolo());
							riga.setQuantitaRichiestaUmPrm(attivita.getOrdineEsecutivo().getQtaOrdinataUMPrm());

							pianoCarico.getRighe().add(riga);
						}
