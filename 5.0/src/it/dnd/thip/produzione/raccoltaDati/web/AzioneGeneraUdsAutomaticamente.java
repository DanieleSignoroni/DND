 ...
 * 71994	10/06/2025	DSSOF3	 Gestione articolo
 */
			...
			for(int i = 0; i < numeroUds; i++) {
				...
				try {
					...
					if(rl != null) {
						...
						uds.setIdArticolo(rl.getCodiceArticolo()); //assuming they are the same
					}
					uds.getDatiComuniEstesi().setStato(DatiComuniEstesi.ANNULLATO); //.La validero' dopo
					uds.setStatoUds(YGestioneUdsPickingProd.PACKING_COMPLETATO);
					int rcUds = uds.save();
					if(rcUds < 0)
						rc = rcUds;
