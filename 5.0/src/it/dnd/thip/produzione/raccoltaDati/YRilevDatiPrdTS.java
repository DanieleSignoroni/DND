...
import it.dnd.thip.base.PsnDatiDnd;
 ...
 * 72020	30/06/2025	DSSOF3	 Modificare metodo checkAtvSuListaPVENL, modificare generazione UDS
 * 72106    04/09/2025  DSSOF3   Interfaccia con Loftware - comando clear su inizio rilevazione.
 */
...
public class YRilevDatiPrdTS extends RilevDatiPrdTS {
	...
	public static final String STMT_IS_ATV_ESEC_PVEN_L = "SELECT 1 FROM THIP.ORD_ESEC OE "
			...
			+ "AND LT.COD_TIPO_LISTA = ? AND LT."+TestataListaTM.STATO_LISTA+" = '"+TestataLista.APERTO+"' ";
	...
	private static final String STMT_UDS_AUTO_GIA_GENERATE = "SELECT "
			+ "	COUNT(*) "
			+ "FROM "
			+ "	THIPPERS.YGESTIONE_UDS_PICKING_PROD "
			+ "WHERE "
			...
			+ "	AND AUTOMATICA = ? "
			+ "	AND NUMERO_RITORNO = ?";
	public static CachedStatement cUdsAutomaticheGiaGenerate = new CachedStatement(STMT_UDS_AUTO_GIA_GENERATE);
	...
	protected String iNumeroRitornoVersamento;
	...
	public String getNumeroRitornoVersamento() {
		return iNumeroRitornoVersamento;
	}

	public void setNumeroRitornoVersamento(String iNumeroRitornoVersamento) {
		this.iNumeroRitornoVersamento = iNumeroRitornoVersamento;
	}
	...
	/**
	 * Durante l'inizio di una nuova rilevazione, gestisco l'interfaccia con Loftware.
	 */
	@Override
	public int SaveInizioRipresa() throws SQLException {
		RilevazioneDatiProdTes rilevazione = cercaRilevazioneSospesa();
		if (getTipoTimbratura() == RilevazioneDatiProdRig.INIZIO
				&& rilevazione == null) {
			int rc = lanciaComandoClearLoftware();
			if(rc < 0) {
				return rc;
			}
		}
		return super.SaveInizioRipresa();
	}

	/**
	 * Durante l'inizio di una nuova rilevazione, se la macchina e' collegata ad una stampante Loftware, mando il comando di clear.
	 */
	public int lanciaComandoClearLoftware() throws SQLException {
		if(getTipoRisorsa() == Risorsa.MACCHINE && getLivelloRisorsa() == Risorsa.MATRICOLA) {
			YMacchina macchina = (YMacchina) YMacchina.elementWithKey(YMacchina.class, 
					KeyHelper.buildObjectKey(new String[] {getIdAzienda(),String.valueOf(getTipoRisorsa()),String.valueOf(getLivelloRisorsa()),getIdRisorsa()
					}), PersistentObject.NO_LOCK);
			if(macchina != null && macchina.getIdStampante1Ti() != null) {
				return YInterfStampanti.sendClearCommandToPrinter(getIdAzienda(),macchina.getIdStampante1Ti());
			}
		}
		return 0;
	}
	...
	public int convalidaRilevazione(RilevazioneDatiProdTes testata, int rc, boolean genDoc) {
		...
		if(rcConvalida > 0 && isAttivitaEsecutivaSuListaPPRE()) {
			...
			if(rcCnfMissioni >= 0)
				rcConvalida += rcCnfMissioni;
			else
				rcConvalida = rcCnfMissioni;
			int rcGesScarto = 0;
			if(PsnDatiDnd.getCurrentPsnDatiDnd() != null) {
				for (int i=0; i < getMateriali().size(); i++) {
					...
					if(materiale != null && materiale.getIdArticolo() != null){
						BigDecimal qtaScartoPrlPrm = (BigDecimal) getQtaScartoUmPrm().get(new Integer(i));
						if(qtaScartoPrlPrm != null && qtaScartoPrlPrm.compareTo(BigDecimal.ZERO) > 0) {
							RilevazioneDatiProdMat rigaRilMat = (RilevazioneDatiProdMat) testata.getRigheMateriali().get(i);
							try {
								AttivitaEsecMateriale atvEsecMat = (AttivitaEsecMateriale) AttivitaEsecMateriale.elementWithKey(AttivitaEsecMateriale.class,
										KeyHelper.buildObjectKey(new String[] {
												...
												rigaRilMat.getIdRigaMateriale().toString()
										}), PersistentObject.NO_LOCK); 
								OperazioneMovimento op = PsnDatiDnd.getCurrentPsnDatiDnd().findOperazioneMovimento(atvEsecMat.getMagazzinoPrelievo());
								if(atvEsecMat != null) {
									List missioni = trovaMissioniAttivitaEsecutivaMateriale(atvEsecMat);
									if(missioni != null && missioni.size() > 0) {
										Missione m = (Missione) missioni.get(0);
										...
										insMov.setOperazMovim(op);
										insMov.setPostazione(m.getPostazione());
										insMov.setOperatore(m.getOperatore());
										insMov.settaCampiOperazMovim();
										insMov.setQuantita(qtaScartoPrlPrm);
										insMov.setQuantitaPrmErp(qtaScartoPrlPrm);
										insMov.setSaldoPrimario(m.getSaldo());
										insMov.setArticolo(m.getArticolo());
										insMov.setLotto(m.getLotto());
										insMov.setCodiceMagLogicoScarico(atvEsecMat.getIdMagazzinoPrl());
										insMov.setCodiceMagLogicoCarico(PsnDatiDnd.getCurrentPsnDatiDnd().getIdMagScartiDichPrel());
										Vector errori = insMov.fine();
										if (errori.size() > 0) {
											rcGesScarto = -1;
											testata.iErrori.addAll(errori);
										}
									}
								}
							}catch (Exception e) {
	...
	protected int confermaMissioniAuto(RilevazioneDatiProdTes testata) {
		...
		while(iterMateriali.hasNext()) {
			...
			try {
				...
				if(atvEsecMat != null) {
					List missioni = trovaMissioniAttivitaEsecutivaMateriale(atvEsecMat);
					for (Iterator iterator = missioni.iterator(); iterator.hasNext();) {
						Missione m = (Missione) iterator.next();
						if(m.getQta1Richiesta().compareTo(materiale.getQtaPrlUmPrm()) == 0) {
							m.setStatoMissione(Missione.ESECUZIONE);
							Vector errori = m.confermaMissione(materiale.getQtaPrlUmPrm(),false);
							if(!errori.isEmpty()) {
								getRilevazioneTesFine().iErrori.addAll(errori);
							}
				...
				for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					if(testataUds != null) {
						...
						if(rcTes > 0) {
							...
							/*Iterator prodotti = getAttivitaEsecutiva().getProdotti().iterator();
							while (prodotti.hasNext()) {
								AttivitaEsecProdotto prodotto = (AttivitaEsecProdotto) prodotti.next();
								rigaUds.setCodiceMagLogico(prodotto.getIdMagazzinoVrs());
							}*/
							//72020 Fine
							rigaUds.setCodiceMagLogico(getAttivitaEsecutiva().getOrdineEsecutivo().getIdMagazzinoVrs());
							//rc = rigaUds.save();
							testataUds.getRigheUds().add(rigaUds);

							udsPicking.getDatiComuniEstesi().setStato(DatiComuniEstesi.VALIDO);
							udsPicking.setCodiceRigaUds(rigaUds.getCodice());
							rc = udsPicking.save();
						}

						//72147 Inizio - Gestione uds Multi Articolo
						String w = " "+YGestioneUdsPickingProdTM.ID_AZIENDA+" = '"+udsPicking.getIdAzienda()+"' "
								+ "AND "+YGestioneUdsPickingProdTM.R_COD_LISTA+" = '"+udsPicking.getIdCodiceLista()+"' "
								+ "AND "+YGestioneUdsPickingProdTM.R_UDS+" = '"+udsPicking.getIdUds()+"' "
								+ "AND NOT ("+YGestioneUdsPickingProdTM.R_UDS+" = '"+udsPicking.getIdUds()+"' "
								+ "AND "+YGestioneUdsPickingProdTM.NUMERO_RITORNO+" = '"+udsPicking.getNumeroRitorno()+"') ";
						Vector udsMA = YGestioneUdsPickingProd.retrieveList(w, "", false);

						for (Iterator iterator2 = udsMA.iterator(); iterator2.hasNext();) {
							YGestioneUdsPickingProd uMA = (YGestioneUdsPickingProd) iterator2.next();

							RigaUds rigaUds = uMA.creaRigaUds(testataUds);
							rigaUds.setCodiceMagLogico(getAttivitaEsecutiva().getOrdineEsecutivo().getIdMagazzinoVrs());

							testataUds.getRigheUds().add(rigaUds);

							uMA.getDatiComuniEstesi().setStato(DatiComuniEstesi.VALIDO);
							uMA.setCodiceRigaUds(rigaUds.getCodice());
							rc = uMA.save();
						}
						rcTes = testataUds.save();
						//72147 Fine 
					}
				}
	...
	public boolean checkAttivitaEsecutivaSuListaPVENL() {
		if(getAttivitaEsecutiva() == null)
			return false;
		if(!getAttivitaEsecutiva().isAttivitaFinale()) //Fix 72020
			return false;
	...
	public ErrorMessage checkUdsGenerateAutomaticamentePresenti(Integer nrScatole) {
		ErrorMessage em = null;
		PreparedStatement ps = null;
		ResultSet resultSet = null;
		try {
			ps = cUdsAutomaticheGiaGenerate.getStatement();
			Database db = ConnectionManager.getCurrentDatabase();
			db.setString(ps, 1, getIdAzienda());
			db.setString(ps, 2, String.valueOf(Column.TRUE_CHAR));
			db.setString(ps, 3, getBollaLavorazione());
			resultSet =  ps.executeQuery();
			if(resultSet.next()){
				Integer nrGenerati = resultSet.getInt(1);
				if(nrGenerati != null && nrGenerati.compareTo(nrScatole) == 0) {
					em = new ErrorMessage("YDND000003",new String[] {nrScatole.toString()});
				}
			}
		}catch (SQLException ex) {
			ex.printStackTrace(Trace.excStream);
