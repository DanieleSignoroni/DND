		...
		if(rl != null) {
			...
			try {
				...
				for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					if(testataUds != null) {
						testataUds.setCodice(udsPicking.getIdUds());
						testataUds.retrieve();
						int rcTes = 1;
						if(!testata.isOnDB()) {
							testataUds.setTipoUds(udsPicking.getTipouds());
							testataUds.setForma(testataUds.getTipoUds().getForma());
							testataUds.getDim().setVolumeIngombro(testataUds.getTipoUds().getDim().getVolumeIngombro());
							testataUds.getDim().setVolumeNetto(testataUds.getTipoUds().getDim().getVolumeNetto());
							testataUds.getDim().setLarghezza(testataUds.getTipoUds().getDim().getLarghezza());
							testataUds.getDim().setLunghezza(testataUds.getTipoUds().getDim().getLunghezza());
							testataUds.getDim().setProfondita(testataUds.getTipoUds().getDim().getProfondita());
							testataUds.getDim().setPesoMax(testataUds.getTipoUds().getDim().getPesoMax());
							testataUds.setTara(testataUds.getTipoUds().getTara());
